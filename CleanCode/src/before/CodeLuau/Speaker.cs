...
namespace CodeLuau
{
	public class Speaker
	{
        ...
        private RegisterError? ValidateRegistration()
        {
            ...
            bool atLeastOneSessionApprovied = ApproveSession();

            if(!atLeastOneSessionApprovied)
        ...
        private bool HasObviousRedFlags()
        {
            ...
            if (domains.Contains(emailDomain)) return true;
        ...
        private bool AppearsExceptional()
        {
            ...
            if (YearsOfExperiences > 10) return true;
            if(HasBlog) return true;
            if (Certifications.Count() > 3 ) return true;
            var perreferdEmployers = new List<string>() { "Pluralsight", "Microsoft", "Google" };
            if (perreferdEmployers.Contains(Employer)) return true;
